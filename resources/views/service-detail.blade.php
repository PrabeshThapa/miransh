...
<head>
    ...
    <meta property="og:title" content="{{ $service->title_ja }} | {{ $company->name_ja ?? 'MIRANSH合同会社' }}">
    <meta property="og:description" content="{{ $service->desc_ja }}">
    <meta property="og:image" content="{{ $service->image_url ? (str_starts_with($service->image_url, 'http') ? $service->image_url : 'https://miransh.co.jp'.$service->image_url) : 'https://miransh.co.jp/images/logo-icon.png' }}">
    <meta property="og:site_name" content="MIRANSH合同会社 (MIRANSH LLC)">
    <meta property="og:locale" content="ja_JP">

    <!-- Twitter Cards -->
    <meta name="twitter:card" content="summary_large_image">
    <meta name="twitter:title" content="{{ $service->title_ja }} | MIRANSH合同会社">
    <meta name="twitter:description" content="{{ $service->desc_ja }}">
    <meta name="twitter:image" content="{{ $service->image_url ? (str_starts_with($service->image_url, 'http') ? $service->image_url : 'https://miransh.co.jp'.$service->image_url) : 'https://miransh.co.jp/images/logo-icon.png' }}">
    ...
    <section class="detail-hero">
        <div class="container">
            ...
            <span class="section-badge" style="background: rgba(255,255,255,0.15); color: #93C5FD; margin-bottom: 12px;">
                SERVICE {{ $service->number_label ?? '01' }}
            </span>
            ...
            <p class="detail-subtitle">
                <span class="lang-ja">{{ $service->subtitle_ja ?? $service->desc_ja }}</span>
                <span class="lang-en">{{ $service->subtitle_en ?? $service->desc_en }}</span>
            </p>
        </div>
    </section>
        ...
        <div class="container">
            <div class="detail-content-layout">
                <div class="detail-main-content">
                    <img src="{{ $service->image ?? '/images/caregiving.jpg' }}" alt="{{ $service->title_ja }}" class="detail-banner-img">
                    ...
                    <div class="detail-prose">
                        <div class="lang-ja">
                            <p style="margin-bottom: 16px;">{{ $service->full_content_ja ?? $service->desc_ja }}</p>
                        </div>
                        <div class="lang-en">
                            <p style="margin-bottom: 16px;">{{ $service->full_content_en ?? $service->desc_en }}</p>
                        </div>
                    </div>
                    ...
                    <div class="workflow-pipeline-box">
                        ...
                        @if (!empty($service->workflow_steps_ja))
                        <div class="lang-ja">
                            @foreach ($service->workflow_steps_ja as $idx => $step)
                            <div class="pipeline-step-item">
                                <div class="pipeline-step-badge">{{ $idx + 1 }}</div>
                                <div class="pipeline-step-text">{{ $step }}</div>
                            </div>
                            @endforeach
                        </div>
                        @endif

                        @if (!empty($service->workflow_steps_en))
                        <div class="lang-en">
                            @foreach ($service->workflow_steps_en as $idx => $step)
                            <div class="pipeline-step-item">
                                <div class="pipeline-step-badge">{{ $idx + 1 }}</div>
                                <div class="pipeline-step-text">{{ $step }}</div>
                            </div>
                ...
                <div>
                    <div style="background: #FFFFFF; border: 1px solid var(--border-color); border-radius: var(--radius-lg); padding: 32px; box-shadow: var(--shadow-md); position: sticky; top: 96px;">
                        ...
                        <div style="background: var(--surface-bg); border-radius: var(--radius-sm); padding: 16px; margin-bottom: 20px;">
                            ...
                            <div style="font-size: 18px; font-weight: 800; color: var(--primary); margin-top: 2px;">{{ $company->phone ?? '042-409-8256' }}</div>
                        </div>
                        ...
                        <div style="margin-top: 36px; border-top: 1px solid var(--border-color); padding-top: 24px;">
                            ...
                            <div style="display: flex; flex-direction: column; gap: 8px;">
                                @foreach ($allServices as $s)
                                    @if ($s->id !== $service->id)
                                    <a href="/services/{{ $s->id }}" style="font-size: 13px; color: #2563EB; font-weight: 600; padding: 6px 0; display: flex; align-items: center; justify-content: space-between;">
                                        <span>
                                            <span class="lang-ja">{{ $s->number_label }}. {{ $s->title_ja }}</span>
                                            <span class="lang-en">{{ $s->number_label }}. {{ $s->title_en }}</span>
                                        </span>
                                        <span>→</span>
                                    </a>
                                    @endif
    ...
    <footer>
        <div class="container">
            <div class="footer-bottom" style="border: none; padding-top: 0;">
                <div>&copy; {{ date('Y') }} {{ $company->name_ja ?? 'MIRANSH合同会社' }} (MIRANSH LLC). All Rights Reserved.</div>
